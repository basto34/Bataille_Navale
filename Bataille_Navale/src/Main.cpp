...
void main() {
				...
				do {
					...
					while (orientation != "h" && orientation != "v" && orientation != "H" && orientation != "V") {
						...
						if (orientation == "h" || orientation == "H") {
							...
							else { std::cout << "T'es idiot ? On t'a dit choisis une valeur possible" << endl; }
						}
					}
	...
	for (int n = 0; n <= 1; n++) {
		...
		std::cout << "Le placement des bateaux verticaux s'effectue de haut en bas, celui des horizontaux de gauche a droite" << endl;
		...
		for (int i = 0; i < nbbateaux; i++) {
			...
			do {
				...
				//Ecriture du placement du bateau x (de taille x) pour le joueur x
				std::cout << "Placement bateau " + to_string(i + 1) + " " + +"(de taille " + to_string(taillebateau) + ")" + " pour le joueur ";
				if (n == 0) {
					std::cout << nom1 << endl;
				}
				else {
				...
				if (n == 0) {
					...
					//On demande la position du bateau
					std::cout << "Entrez le point d'origine du premier bateau (x,y) sachant que x est une lettre (A a J) et y un nombre (1 a 10) : " << endl;
					std::cin >> lettre >> y;
					...
					isHorizontal = joueur2.horizontalBoat();
					x = joueur2.posBoat()[0];
					y = joueur2.posBoat()[1];
				}
				//On initialise le bateau
				...
				if (n == 0) {
					if (grille1.estBienPlace(boat)) {
						std::cout << "Le bateau est bien place" << endl;
						test = true;
						grille1.poseBoat(boat);
						grille1.afficherGrille();
						system("pause");
						...
						std::cout << "Le bateau ne peut pas etre place ici. Recommencez" << endl;
					}
	...
	while (!grille1.estVide() || !grille2.estVide()) {
		...
		//Tour du joueur 1 
		std::cout << "Tour de " + nom1 << endl;
		char lettre1;
		int y1;
		std::cout << "Grille de l'adversaire" << endl;
		grille3.afficherGrille();
		std::cout << "-------------------------------------------------------------------------" << endl;
		std::cout << "Votre grille" << endl;
		grille1.afficherGrille();
		std::cout << nom1 + " donnez votre coup (x,y) avec sachant que x est une lettre (A a J) et y un nombre (1 a 10) : " << endl;
		std::cin >> lettre1;
		std::cin >> y1;

		while ((int(lettre1) <= 64 || (int(lettre1) > 74 && int(lettre1) < 97) || int(lettre1) > 106) || (y1 < 1 || y1 > 10))
		{
			cout << "Les valeurs ne sont pas bonnes ! Rentrez la colonne entre A et J puis la ligne entre 1 et 10" << endl;
			cin >> lettre1 >> y1;
		}
		...
		std::cout << "Votre grille" << endl;
		grille2.afficherGrille();
		//std::cout << nom2 + " donnez votre coup (x,y) avec sachant que x est une lettre (A a J) et y un nombre (1 a 10) : " << endl;
		vector<int> jeuIA = joueur2.jouerReflechi(grille1);
		...
		//On frappe la grille adverse
		int touche2 = grille1.toucherPos(y2, x2);
		system("CLS");
		std::cout << "Coup envoye !" << endl;
		for (int i = 0; i <= 2; i++) {
			std::this_thread::sleep_for(250ms);
		...
		switch (touche2) {
		case -1:
			std::cout << "Dans l'eau :(" << endl;
			break;
		case -2:
			std::cout << "Touche !!!" << endl;
			break;
		default:
			std::cout << "Bizarre..." << endl;
			break;
		}
		std::this_thread::sleep_for(1500ms);
		//On note le résultat sur la grille de l'adversaire
